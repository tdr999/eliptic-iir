...
class M4(device: BluetoothDevice) {
    ...
    val gattCallBack = object : BluetoothGattCallback() {
            ...
            with(characteristic) {
                ...
                if (valoare.size == 20) {
                    ...
                    if (valoare[6] == "0C ") {
                        ...
                        distance =
                            (valoare[13].trim() + valoare[14].trim() + valoare[15].trim() + valoare[16].trim()).toInt(
                                16
                            ).toFloat() / 1000
                        steps =
                            (valoare[9].trim() + valoare[10].trim() + valoare[11].trim() + valoare[12].trim()).toInt(
                                16
                            )
        ...
        override fun onCharacteristicWrite(
            ...
            Log.i(
                "written to charac",
                "${characteristic?.uuid}, ${characteristic?.value?.toHexString()}"
            )
        }
        ...
        override fun onServicesDiscovered(gatt: BluetoothGatt?, status: Int) {
            ...
            val caracteristicaNotificari =
                serviciuDeConectare?.getCharacteristic(UUID.fromString("6e400003-b5a3-f393-e0a9-e50e24dcca9f"))
            val caracteristicaComenzi =
                serviciuDeConectare?.getCharacteristic(UUID.fromString("6e400002-b5a3-f393-e0a9-e50e24dcca9f"))
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                caracteristicaComenzi?.value = byteArrayOf(
                    223.toByte(),
                    0x00,
                    0x0d,
                    214.toByte(),
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                caracteristicaComenzi?.value =
                    byteArrayOf(253.toByte(), 0x00, 0x05, 0x17, 0x0c, 0x08, 0x00, 0x00, 0x01)
                gatt?.writeCharacteristic(caracteristicaComenzi)
            }, 2125)

            Handler(Looper.getMainLooper()).postDelayed({
                caracteristicaComenzi?.value =
                    byteArrayOf(223.toByte(), 0x00, 0x06, 0x04, 0x02, 0x10, 0x0a, 0x00, 0x01, 0x02)
                gatt?.writeCharacteristic(caracteristicaComenzi)
            }, 2250)
//
...
//            }, 2375) //dupa multe teste, am ajuns la conculzia ca aceasta linie schimba  macar ora , dar nu stim cum asa ca vom seta cu aplicati ainitiala
            ...
            Handler(Looper.getMainLooper()).postDelayed({
                caracteristicaComenzi?.value =
                    byteArrayOf(253.toByte(), 0x00, 0x05, 0x15, 0x09, 0x09, 0x00, 0x00, 0x01)
                gatt?.writeCharacteristic(caracteristicaComenzi)
            }, 2875)

            Handler(Looper.getMainLooper()).postDelayed({
                caracteristicaComenzi?.value =
                    byteArrayOf(223.toByte(), 0x00, 0x05, 0x04, 0x0f, 0x10, 0x01, 0x00, 0x00)
                gatt?.writeCharacteristic(caracteristicaComenzi)
            }, 3000)

            Handler(Looper.getMainLooper()).postDelayed({
                caracteristicaComenzi?.value =
                    byteArrayOf(253.toByte(), 0x00, 0x05, 0x1b, 0x0f, 0x09, 0x00, 0x00, 0x01)
                gatt?.writeCharacteristic(caracteristicaComenzi)
            }, 3125)
    ...
    fun getSteps() {
        ...
        Handler(Looper.getMainLooper()).postDelayed({
            car_com?.value =
                byteArrayOf(253.toByte(), 0x00, 0x05, 0x17, 0x0f, 0x05, 0x00, 0x00, 0x01)
            gatt?.writeCharacteristic(car_com)
        }, 125)

        Handler(Looper.getMainLooper()).postDelayed({
            car_com?.value =
                byteArrayOf(223.toByte(), 0x00, 0x06, 0x02, 0x05, 0x10, 0x06, 0x00, 0x01, 0x01)
            gatt?.writeCharacteristic(car_com)
    ...
    fun getHeart() {
        ...
        var bytes_to_write_hrt = byteArrayOf(
            //comanda ptr masurare puls, log hrt tarziu
            223.toByte(),
            0.toByte(),
            6.toByte(),
            6.toByte(),
    ...
    fun sendMessage() {
        ...
        var serviciu_mesaj =
            gatt?.getService(UUID.fromString("6e400001-b5a3-f393-e0a9-e50e24dcca9f"))
        var caracter_mesaj =
            serviciu_mesaj?.getCharacteristic(UUID.fromString("6e400002-b5a3-f393-e0a9-e50e24dcca9f"))
...
//            114.toByte(),
        ...
        Handler(Looper.getMainLooper()).postDelayed(
            {
                caracter_mesaj?.value = bytes_to_write
        ...
        Handler(Looper.getMainLooper()).postDelayed(
            {
                caracter_mesaj?.value = mesaj_bytes
                gatt?.writeCharacteristic(caracter_mesaj)
    ...
    fun getBlood() {
        ...
        var bytes_to_write = byteArrayOf(
            //comanda tensiune ultimii 2 bytes
            223.toByte(),
            0.toByte(),
            6.toByte(),
            7.toByte(),
    ...
    fun saveMeasurements() {
        globalDatabase.db.insertMeasurement(
            ...
            saturation,
            0,
            (pressure2.toString() + "/" + pressure1.toString()),
            steps,
            distance,
            calories,
